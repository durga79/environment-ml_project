...
\begin{abstract}
Environmental monitoring faces growing challenges as data volumes increase from sensor networks and policy documents. This study explores how machine learning can address two specific problems: predicting air quality index categories from sensor measurements and classifying sentiment in climate policy texts. We tested four algorithms—Random Forest, XGBoost, Logistic Regression, and Support Vector Machines—following the CRISP-DM framework. Our air quality dataset contained 15,000 hourly measurements, while the text dataset included 9,500 climate policy documents. For text analysis, we used TF-IDF to convert documents into numerical features, enabling classification into positive, neutral, or negative sentiment categories. The air quality models focused on identifying which pollutants and weather conditions most strongly predict AQI categories.

Our findings show Random Forest slightly outperformed XGBoost for air quality prediction, reaching 98.7\% accuracy with an F1-score of 0.987. Interestingly, both Logistic Regression and SVM achieved 100\% accuracy on the sentiment classification task, likely due to the synthetic nature of our data where classes are well-separated. We used SHAP and LIME to understand model decisions, revealing that PM2.5, PM10, and NO2 are the strongest predictors of air quality, while specific vocabulary patterns drive sentiment classifications in policy documents.
\end{abstract}
...
Environmental issues like air pollution and climate change continue to worsen, creating massive amounts of data from monitoring stations and policy archives. Machine learning offers a way to make sense of this information, helping agencies track pollution levels and understand how people respond to climate policies \cite{chapman2000crisp}. Our work tackles two specific problems that matter for public health and effective policy-making.

First, we wanted to see if we could automatically predict air quality categories from sensor readings. Monitoring stations produce thousands of measurements every hour, so manual checking isn't practical. Machine learning could enable real-time alerts and trend tracking. Second, we explored whether we could classify the sentiment of climate policy documents—essentially understanding whether policies are viewed positively, negatively, or neutrally. This could help policymakers see how their initiatives are being received and adjust their messaging.
...
\begin{enumerate}
    \item Compare how well different machine learning approaches work on both structured air quality data and unstructured text data.
    ...
    \item Figure out which factors matter most for predicting air quality and sentiment.
